...
public class OrderController {
    ...
    @GetMapping("/{id}") public OrderEntity get(@PathVariable UUID id) { return svc.get(id); }

    @PostMapping public OrderEntity create(@Valid @RequestBody OrderCreateRequest req) { return svc.create(req); }

    @PatchMapping("/{id}") public OrderEntity update(@PathVariable UUID id, @RequestBody OrderUpdateRequest req) {
        return svc.update(id, req);
    }

    @PostMapping("/{id}/cancel") public OrderEntity cancel(@PathVariable UUID id) { return svc.cancel(id); }

    @DeleteMapping("/{id}") public ResponseEntity<?> delete(@PathVariable UUID id) {
